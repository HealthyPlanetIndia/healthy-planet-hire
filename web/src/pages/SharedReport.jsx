...
export default function SharedReport() {
  ...
  return <Wrap>
    ...
    {evals.length > 0 && <div className="card"><b>Panel scores</b>{d.rubric.map((r) => { const v = evals.map((e) => +e.scores?.[r.key]).filter(Boolean); return <div key={r.key} className="row" style={{ justifyContent: "space-between", fontSize: 13, marginTop: 4 }}><span className="muted">{r.label}</span><b>{v.length ? (v.reduce((a, b) => a + b, 0) / v.length).toFixed(1) : "not scored"} / 5</b></div>; })}
