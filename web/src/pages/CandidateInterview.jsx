...
export default function CandidateInterview() {
  ...
  function beginAnswer() {
    ...
    const cap = info?.answer_seconds || 90;
      ...
      {!running && (
        <div style={{ padding: 16, maxWidth: 720, width: "100%", margin: "0 auto" }}>
          <div className="card" style={{ background: "#fff", color: "var(--ink)" }}>
            ...
            <p style={{ lineHeight: 1.55, marginTop: 6 }}>{lang === "hi" ? `यह ${info.role} पद के लिए चयन प्रक्रिया का पहला दौर है। माया, हमारी साक्षात्कारकर्ता, ${info.total} सवाल पूछेंगी, साथ में कुछ फ़ॉलो-अप${info.interactive ? " और एक छोटा रोल-प्ले" : ""}। लगभग 12 से 15 मिनट; हर जवाब के लिए अधिकतम ${info.answer_seconds || 90} सेकंड, और बोलते समय टाइमर दिखता है। आपके जवाबों को इस भूमिका की ज़रूरतों के अनुसार परखा जाता है, और इसी से तय होता है कि स्कूल में पैनल दौर के लिए किसे बुलाया जाए।` : `This is the first round of the selection process for ${info.role}. Maya, our interviewer, will ask ${info.total} questions with some follow-ups${info.interactive ? " and a short role play in which she plays another person" : ""}. It takes about 12 to 15 minutes; each answer has up to ${info.answer_seconds || 90} seconds, and the timer is shown while you speak. Your answers are assessed against what this role needs, and the result decides who is invited to the panel rounds at the school. Specific examples from your own classroom count for more than general statements, and nobody expects polish.`}</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="card" style={{ background: "#fff", color: "var(--ink)", flex: 1 }}>
              ...
              {phase === "answering" && <div style={{ marginTop: 12 }}>
                <div className="row" style={{ justifyContent: "space-between" }}><div style={{ fontSize: 13, color: "#C0392B", fontWeight: 700 }}>● {t.rec}{isRetake ? ` · ${t.retaking}` : ""}</div><div className="muted" style={{ fontSize: 12 }}>{Math.max(0, (info?.answer_seconds || 90) - seconds)}s left</div></div>
                <div style={{ height: 4, background: "var(--soft)", borderRadius: 2, marginTop: 6 }}><div style={{ height: 4, width: `${Math.min(100, seconds / (info?.answer_seconds || 90) * 100)}%`, background: seconds > (info?.answer_seconds || 90) * 0.8 ? "#C0392B" : "var(--green)", borderRadius: 2, transition: "width .5s" }} /></div>
