...
import { ScrollStrip } from "../components/ui.jsx";
...
function RolesBoard({ d }) {
  const [campus, setCampus] = useState(""); const [showClosed, setShowClosed] = useState(false); const wrapRef = React.useRef(null);
    ...
    <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12, minWidth: 900 }}>
      ...
      <tbody>
        {Object.entries(byCampus).map(([c, rs]) => <React.Fragment key={c}>
          <tr><td colSpan={COLS.length + 5} style={{ padding: "10px 4px 4px", fontWeight: 700, color: "var(--green)", borderTop: "1px solid var(--line)" }}>{c} <span className="muted" style={{ fontWeight: 400 }}>· {rs.length} role{rs.length > 1 ? "s" : ""} · {rs.reduce((a, r) => a + r.total, 0)} applications</span></td></tr>
