  ...
  return (
    <div className="grid" style={{ maxWidth: 800 }}>
      <div className="card">
        ...
        {Object.entries(t).map(([k, v]) => <Field key={k} label={{ followup: "Follow-up nudge", interview_reminder: "Interview reminder (automatic)", offer_letter: "Offer letter ({name}, {role}, {department}, {campus}, {salary}, {join_date}, {date}, {ref_no}, {reporting_manager})", appointment_letter: "Appointment letter", needs_human: "Assistant hand-off reply", followup_2: "Follow-up nudge, second", followup_3: "Follow-up nudge, third" }[k] || `When moved to ${k}`}><textarea style={{ minHeight: k === "offer_letter" ? 220 : 70 }} value={v} onChange={(e) => setT({ ...t, [k]: e.target.value })} /></Field>)}
