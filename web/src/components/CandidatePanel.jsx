      ...
      <aside className="drawer" onClick={(e) => e.stopPropagation()}>
        ...
        {tab === "report" && <div className="grid">
          ...
          {iv && <div className="card" style={{ borderColor: "var(--blue)" }}>
            ...
            {!iv.report && <div className="muted" style={{ fontSize: 13, marginTop: 4 }}>{iv.status === "pending" ? `Link sent, not yet opened. Expires ${new Date(iv.expires_at).toLocaleDateString("en-IN")}.` : iv.status === "in_progress" ? "Candidate is mid-interview." : "Report is being written; it normally takes one to two minutes."}<br /><span style={{ wordBreak: "break-all" }}>{iv.link}</span>
              {iv.status === "completed" && !isManager() && <div style={{ marginTop: 8 }}><button className="small primary" disabled={busy === "rep"} onClick={() => run("rep", async () => { await api(`/candidates/${id}/interviews/${iv.id}/rereport`, { method: "POST", body: {} }); say("Report generated"); })}>{busy === "rep" ? "Writing report (30 to 60 s)..." : "Generate report now"}</button><div className="muted" style={{ fontSize: 12, marginTop: 4 }}>If the report has not appeared after a few minutes, press this. The History tab shows the reason if the first attempt failed.</div></div>}</div>}
            {iv.questions?.length > 0 && <details style={{ fontSize: 12, marginTop: 6 }}><summary className="muted">Questions drawn for this interview{iv.scenario ? ` and the scenario "${iv.scenario.title}"` : ""}</summary><ol style={{ margin: "4px 0 0", paddingLeft: 18 }}>{iv.questions.map((q, i) => <li key={i}>{q.text} <span className="muted">({q.assesses})</span></li>)}</ol>{iv.scenario && <div style={{ marginTop: 4 }}><b>{iv.scenario.type === "roleplay" ? "Role play" : "Situation"}:</b> {iv.scenario.surface || iv.scenario.setup}{iv.scenario.underlying && <div className="muted">Underlying issue: {iv.scenario.underlying}</div>}{iv.scenario.observe && <div className="muted">Looking for: {iv.scenario.observe}</div>}</div>}</details>}
  ...
  return <div style={{ marginTop: 10, borderTop: "1px solid var(--line)", paddingTop: 10 }}>
    ...
    {open && clips && <div className="grid" style={{ gap: 12, marginTop: 8 }}>
      {clips.map((k) => <div key={k.index} style={{ fontSize: 13 }}>
        ...
        {k.has_video ? <video controls preload="metadata" src={k.url} style={{ width: "100%", maxHeight: 280, background: "#000", borderRadius: 8 }} /> : <div className="muted" style={{ fontSize: 12, background: "var(--soft)", borderRadius: 8, padding: 8 }}>Video for this answer did not finish uploading on the candidate's connection; the audio was received and transcribed.</div>}
...
function ReportV2({ iv, c, say, reload }) {
  ...
  return <div style={{ marginTop: 6, fontSize: 13 }}>
    ...
    {!isManager() && <div style={{ marginTop: 10, borderTop: "1px solid var(--line)", paddingTop: 8 }}><div className="muted" style={{ fontSize: 12, marginBottom: 4 }}>Rewrite the report under the current assessment rules (also re-runs the integrity check). Optionally add what you heard or saw in the recording first, and it is taken into account.</div><div className="row"><input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Optional: e.g. In Q3 she clearly said 'rapport', not 'rapper'; the example was for Grade 4" style={{ flex: 1 }} /><button className="small primary" disabled={busy} onClick={async () => { setBusy(true); try { await api(`/candidates/${c.id}/interviews/${iv.id}/rereport`, { method: "POST", body: { note } }); say(note.trim() ? "Report rewritten with your note" : "Report rewritten"); setNote(""); reload(); } catch (e) { say(e.message); } setBusy(false); }}>{busy ? "Rewriting (30 to 60 s)..." : "Rewrite report"}</button></div></div>}
