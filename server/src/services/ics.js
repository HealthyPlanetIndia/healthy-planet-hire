...
export function ics({ title, starts_at, ends_at, location, description = "", uid }) {
  return ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Healthy Planet Hire//EN", "BEGIN:VEVENT", `UID:${uid}@healthyplanetschool`, `DTSTAMP:${fmt(new Date())}`, `DTSTART:${fmt(starts_at)}`, `DTEND:${fmt(ends_at)}`, `SUMMARY:${title}`, `LOCATION:${location}`, `DESCRIPTION:${description.replace(/\n/g, "\\n")}`, "END:VEVENT", "END:VCALENDAR"].join("\r\n");
}
